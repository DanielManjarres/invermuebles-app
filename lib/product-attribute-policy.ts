...
type ProductAttributeDefinition = {
  ...
  id: string;
  name: string;
  options: Array<{ active: boolean; id: string; value: string }>;
  required: boolean;
};
...
export function normalizeProductReference(value?: string) {
  return cleanText(value).toUpperCase();
}

export function validateProductInventoryInput(input: {
  cost?: number;
  minimumStock?: number;
  name?: string;
  reference?: string;
  ...
  if (name.length < 2 || name.length > 100) {
    return "El nombre del producto debe tener entre 2 y 100 caracteres.";
  }
...
export function normalizeProductAttributes(
  ...
  inputs: ProductAttributeInput[],
) {
  const inputByAttribute = new Map<string, ProductAttributeInput>();
  for (const input of inputs) {
    if (inputByAttribute.has(input.attributeId)) {
      return { error: "No repitas características en el mismo producto.", values: [] };
    }
  ...
  if (inputs.some((input) => !definitionById.has(input.attributeId))) {
    return { error: "Una de las características no pertenece al tipo de producto.", values: [] };
  }

  const values: NormalizedProductAttribute[] = [];
  for (const definition of definitions.filter((item) => item.active)) {
    ...
    if (!input || (!rawValue && !input.optionId)) {
      if (definition.required) {
        return { error: `Completa la característica obligatoria ${definition.name}.`, values: [] };
      }
      continue;
    }

    if (definition.dataType === "OPTION") {
      const option = definition.options.find((item) => item.id === input.optionId && item.active);
      if (!option) {
        return { error: `Selecciona una opción válida para ${definition.name}.`, values: [] };
      }
      values.push({ attributeId: definition.id, optionId: option.id, value: option.value });
      continue;
    }
    ...
    if (definition.dataType === "NUMBER") {
      ...
      if (!Number.isFinite(numberValue)) {
        return { error: `${definition.name} debe contener un número válido.`, values: [] };
      }
      values.push({ attributeId: definition.id, optionId: null, value: String(numberValue) });
      continue;
    }
    ...
    if (definition.dataType === "BOOLEAN") {
      ...
      if (booleanValue !== "true" && booleanValue !== "false") {
        return { error: `${definition.name} debe indicar verdadero o falso.`, values: [] };
      }
      values.push({ attributeId: definition.id, optionId: null, value: booleanValue });
      continue;
    }

    values.push({ attributeId: definition.id, optionId: null, value: rawValue });
  }
